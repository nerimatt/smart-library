...
from accurate_time import get_timezone

# NOTE: for this to work, time needs to be set first, via set_time
def get_italy_timezone() -> timezone:
    timezone(timedelta(hours = get_timezone()), "italy")
...
if __name__ == "__main__":
    ...
    future = timedelta(hours = 4)

    print(f"timezone: +{get_timezone()}")
    tz = timezone(timedelta(hours = get_timezone()), "italy")
    n = datetime.fromtimestamp(_t.time(), tz) # from unix time
